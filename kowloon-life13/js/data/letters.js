...
export const LETTERS = [
  ...
  // ---------- v8追加: 映画館の館長 星影から ----------
  L('LTR_HOSHIKAGE', {
    sender: 'HOSHIKAGE', trigger: 'flag(FLG_MET_HOSHIKAGE)', onceFlag: true,
    slots: {
      GREETING: [{ cond: 'true', text: '拝啓、大家さん。名画は手紙のように残る……というわけで、わしも一筆したためたのじゃ。' }],
      BODY: [{ cond: 'true', text: 'この街に映画館を建ててくれた恩は、フィルム千巻ぶんじゃ。暗闇の中で夢を見る場所を、わしはずっと守っていくぞ。' }],
      CONTEXT: [
        { cond: 'weather == RAIN', text: '今日みたいな雨の日は、来館者が増えてのう。傘を畳んで暗闇に入る、あの背徳感がたまらんのじゃろう。' },
        { cond: 'true', text: '床屋の親父と郵便屋君は、わしの映画仲間じゃ。今度、おぬしも交えて上映会をやろうではないか。' },
      ],
      CLOSING: [{ cond: 'true', text: 'それでは、また銀幕の前で。 シネマ・ホシカゲ館長 星影幸太郎' }],
    },
    attachCoin: 2,
    replies: [{ key: 'HAPPY', label: '観に行くよ' }, { key: 'TEASE', label: '語りが長い' }, { key: 'WORRY', label: '働きすぎ注意' }],
    effects: ['aff(HOSHIKAGE,2)'],
  }),

  // ---------- v8追加: ムニのひな祭りの手紙 ----------
  L('LTR_MUNI_HINA', {
    sender: 'MUNI', trigger: 'rank(PC, MUNI) >= 3 and season == SPRING', oncePerSeason: true,
    slots: {
      GREETING: [{ cond: 'true', text: 'おおやさんへ。ムニ、じぶんでおてがみかいたの!' }],
      BODY: [{ cond: 'true', text: 'きょう、しゅうかいじょでおひなさま、みたの! およめさんのおひなさま、すっごくきれいだったの! ムニもおおきくなったら、およめさんになるの!' }],
      CLOSING: [{ cond: 'true', text: 'また、いっしょにおまつりしようね。 ムニより' }],
    },
    attachCoin: 1,
    replies: [{ key: 'HAPPY', label: 'かわいいね' }, { key: 'TEASE', label: '字がじょうず' }, { key: 'WORRY', label: '食べすぎ注意' }],
    effects: ['aff(MUNI,2)'],
  }),

  // ---------- v8追加: 夏祭りの夜のヒュウ ----------
  L('LTR_HYU_NATSU', {
    sender: 'HYU', trigger: 'rank(PC, HYU) >= 3 and season == SUMMER', oncePerSeason: true,
    slots: {
      GREETING: [{ cond: 'true', text: '拝啓、大家さん。夏祭りの提灯の灯りに照らされて、この手紙を書いています。' }],
      BODY: [{ cond: 'true', text: 'りんご飴の赤い艶と、私の唇。どちらが美しいか競いましたが……今宵は引き分けにしておきましょう。祭りの夜は、無粋な勝負は似合いませんからね。' }],
      CLOSING: [{ cond: 'true', text: '来年の祭りも、隣で。 ヒュウ' }],
    },
    attachCoin: 1,
    replies: [{ key: 'HAPPY', label: 'きれいだった' }, { key: 'TEASE', label: '自信家め' }, { key: 'WORRY', label: '夜更かし注意' }],
    effects: ['aff(HYU,1)'],
  }),

  // ---------- v8追加: 誕生日を祝われたネオの礼状 ----------
  L('LTR_NEO_BIRTHDAY', {
    sender: 'NEO', trigger: 'rank(PC, NEO) >= 4 and season == WINTER', oncePerSeason: true,
    slots: {
      GREETING: [{ cond: 'true', text: '大家殿へ。我が生誕を祝ってもらった礼を、騎士の作法に則り、書状にて。' }],
      BODY: [{ cond: 'true', text: '故郷を出て以来、誕生日を祝われたのは初めてであった。……この街が、我が新たな故郷になりつつある。それも、貴殿のおかげである。感謝する。' }],
      CONTEXT: [{ cond: 'info(INFO_NEO_PAST_2)', text: 'いつか胸を張って故郷に帰る日が来ても……この塔のことは、生涯忘れぬ。誓おう。' }],
      CLOSING: [{ cond: 'true', text: '貴殿に、騎士ネオの忠義を。 ネオ' }],
    },
    attachCoin: 2,
    replies: [{ key: 'HAPPY', label: 'おめでとう' }, { key: 'TEASE', label: '大げさだよ' }, { key: 'WORRY', label: '無理しないで' }],
    effects: ['aff(NEO,2)'],
  }),

  // ---------- v8追加: サチとリンの連名の便り ----------
  L('LTR_SACHI_RIN', {
    sender: 'CAFEGIRL', trigger: 'flag(FLG_MET_CLERK) and rank(PC, CAFEGIRL) >= 3', onceFlag: true,
    slots: {
      GREETING: [{ cond: 'true', text: '大家さんへ♪ サチとリンちゃん、連名でお手紙です!' }],
      BODY: [{ cond: 'true', text: 'リンちゃんが休憩でお店に来てくれるようになって、ガールズトークが楽しくて仕方ないんです。恋バナに、お店の愚痴に……看板娘どうし、すっかり親友になりました♪' }],
      CONTEXT: [{ cond: 'true', text: '「恋人にするなら誰?」なんて話でも盛り上がってます。……大家さんの名前も、ちょっとだけ出たとか、出ないとか。ふふ、秘密です♪' }],
      CLOSING: [{ cond: 'true', text: 'またお店にも来てくださいね。 サチ&リン' }],
    },
    attachCoin: 1,
    replies: [{ key: 'HAPPY', label: '仲良しだね' }, { key: 'TEASE', label: '気になる…' }, { key: 'WORRY', label: '働きすぎ注意' }],
    effects: ['aff(CAFEGIRL,1)', 'aff(CLERK,1)'],
  }),

  // ---------- v8追加: 怪談クラブの夜のゲル ----------
  L('LTR_GERU_KAIDAN', {
    sender: 'GERU', trigger: 'rank(PC, GERU) >= 4 and season == SUMMER', oncePerSeason: true,
    slots: {
      GREETING: [{ cond: 'true', text: '大家へ。……柄でもないが、夏の夜に一筆。' }],
      BODY: [{ cond: 'true', text: '怪談クラブの夜、私が一番怖い話をしてやったら、ジンパチが半泣きでな。……ふ、ああいう夜も悪くない。人が集まる場所は、いいものだ。この街が、ずっと続けばいい。' }],
      CLOSING: [{ cond: 'true', text: '灯りを消して、また会おう。 ゲル' }],
    },
    attachCoin: 1,
    replies: [{ key: 'HAPPY', label: '楽しそう' }, { key: 'TEASE', label: 'ジンパチ可哀想' }, { key: 'WORRY', label: '夜更かし注意' }],
    effects: ['aff(GERU,1)'],
  }),

  // ---------- v13追加: こどもの日のジンパチ ----------
  L('LTR_JIN_KODOMO', {
    sender: 'JIN', trigger: 'rank(PC, JIN) >= 3 and season == SUMMER', oncePerSeason: true,
    slots: {
      GREETING: [{ cond: 'true', text: '大家! こどもの日だな! 集会所の鯉のぼり、見たか!' }],
      BODY: [{ cond: 'true', text: '一番でかい真鯉、俺が揚げるの手伝ったんだ! ムニのやつが「いちばんちっちゃいのがムニ」って喜んでてよ、なんか俺まで嬉しくなっちまった。あいつが、でっかく育つといいよな!' }],
      CLOSING: [{ cond: 'true', text: '柏餅、お前のぶんも取っといたぜ! ジンパチ' }],
    },
    attachCoin: 1,
    replies: [{ key: 'HAPPY', label: 'いい話だ' }, { key: 'TEASE', label: '柏餅ありがと' }, { key: 'WORRY', label: '食べ過ぎ注意' }],
    effects: ['aff(JIN,1)'],
  }),

  // ---------- v13追加: 花火大会の夜のヒュウ ----------
  L('LTR_HYU_HANABI', {
    sender: 'HYU', trigger: 'rank(PC, HYU) >= 4 and season == SUMMER', oncePerSeason: true,
    slots: {
      GREETING: [{ cond: 'true', text: '拝啓、大家さん。花火大会の夜に、屋上からこの手紙を書いています。' }],
      BODY: [{ cond: 'true', text: '夜空に一瞬だけ咲く大輪の花。……儚いからこそ美しい、とはよく言ったものです。私の美は永遠を目指しますが、あの花火の潔さには、少しだけ憧れます。' }],
      CONTEXT: [{ cond: 'true', text: '屋上でみんなと見上げる花火は、格別でした。……こういう夜が、この街にずっと続けばいいのに、と柄にもなく思いましたよ。' }],
      CLOSING: [{ cond: 'true', text: 'また来年、隣で。 ヒュウ' }],
    },
    attachCoin: 2,
    replies: [{ key: 'HAPPY', label: 'きれいだった' }, { key: 'TEASE', label: '素直だね' }, { key: 'WORRY', label: '夜風に注意' }],
    effects: ['aff(HYU,1)'],
  }),

  // ---------- v13追加: 花火を見たネオ ----------
  L('LTR_NEO_HANABI', {
    sender: 'NEO', trigger: 'rank(PC, NEO) >= 3 and season == SUMMER', oncePerSeason: true,
    slots: {
      GREETING: [{ cond: 'true', text: '大家殿へ。花火大会の夜の感慨を、書状にてしたためる。' }],
      BODY: [{ cond: 'true', text: '天を焦がす火の花……我が故郷の魔法の炎を思い出した。だが、あれは誰も傷つけぬ、平和の火であった。……この街で見た花火を、我は生涯忘れぬであろう。' }],
      CLOSING: [{ cond: 'true', text: '共に見上げた夜空に、感謝を。 ネオ' }],
    },
    attachCoin: 1,
    replies: [{ key: 'HAPPY', label: '感動したね' }, { key: 'TEASE', label: '詩人だね' }, { key: 'WORRY', label: '無理しないで' }],
    effects: ['aff(NEO,1)'],
  }),
];
