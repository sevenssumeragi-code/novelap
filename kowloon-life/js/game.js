...
import { CHARS, MAIN_IDS, ALL_IDS, RANKS, rankOf } from './data/chars.js';
import { FACILITIES, INITIAL_TOWER, ECON, isFacilityOpen } from './data/facilities.js';
...
// セーブ
// ============================================================
const SAVE_KEY = 'kowloon_life_save_v1';
...
function respawnCoins() {
  ...
  // シャッフル(決定論)
  const shuffled = [...slots].sort(() => rng() - 0.5).slice(0, ECON.coinHiddenPerDay);
...
clock.on('minute', (m) => {
  if (m % 10 === 0) {
    updateNpcPositions();
    ambient.scan(Object.fromEntries(ALL_IDS.map(id => [id, { loc: schedule.locationOf(id), activity: schedule.activityOf(id) }])), game);
  }
});
...
function focusLoc(locId) {
  ...
  setHint('住人クリックで会話 / 光るコインを探そう / Escで戻る');
}
...
function showBarkAbove(charId, text, ttl = 4) {
  const n = npcs[charId];
  if (!n) return;
  const sp = makeBarkSprite(text);
  const h = 2.4 * (CHARS[charId].scale ?? 1);
  sp.position.set(0, h, 0);
  n.mesh.add(sp);
  activeBarks.push({ sprite: sp, mesh: n.mesh, ttl });
}
...
function tryRandomBark() {
  const visible = ALL_IDS.filter(id => npcVisibleToPlayer(id));
...
function handleClick(ev) {
  ...
  const npcHits = ray.intersectObjects(ALL_IDS.map(id => npcs[id].mesh), true);
  if (npcHits.length) {
...
function closeModals() {
  for (const id of ['gacha-modal', 'build-modal', 'help-modal', 'dict-modal', 'letters-modal', 'log-modal']) el(id).style.display = 'none';
}
...
function animate() {
  ...
  for (let i = activeBarks.length - 1; i >= 0; i--) {
    ...
    b.ttl -= dt;
    if (b.ttl <= 0) {
      b.mesh.remove(b.sprite);
      b.sprite.material.map.dispose();
      b.sprite.material.dispose();
      activeBarks.splice(i, 1);
    }
  }
  ...
  if (ambient.active && ambientShownIdx !== ambient.active.lineIdx) {
    ...
    if (line && npcVisibleToPlayer(line.sp)) showBarkAbove(line.sp, line.text, 3.6);
  }
