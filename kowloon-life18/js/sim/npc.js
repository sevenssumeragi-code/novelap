...
export class ScheduleEngine {
  ...
  homeOf(charId) {
    const facHome = { BARBER: 'barber', POSTMAN: 'post', CAFEGIRL: 'cafe', DAGASHIYA: 'dagashi', THIEF: 'konbini', CLERK: 'konbini', MARTIN: 'church', HOSHIKAGE: 'cinema' }[charId];
  ...
  update() {
    ...
    // --- 季節イベント: 会場にみんなで集まる(カネは仲良ければ) ---
    //     venue: 既定は集会所(hall)。屋上(ROOF)/路地(GROUND)の催しも可。
    const ev = this.game.currentEvent?.();
    const venue = ev?.venue ?? 'hall';
    const venueReady = (venue === 'ROOF' || venue === 'GROUND') ? true : this.game.isBuilt(venue);
    const bands = ev?.bands ?? EVENT_GATHER_BANDS;
    if (ev && venueReady && bands.includes(this.world.clock.band)) {
      for (const id of MAIN_IDS) this.states[id] = { loc: venue, activity: 'GATHER' };
      if (this.world.kaneSoftness >= 40) this.states.KANE = { loc: venue, activity: 'GATHER' };
    } else if (ev?.id === 'CHRISTMAS' && this.world.clock.band === 'MIDNIGHT') {
      // --- クリスマスの深夜: 5人のうち誰かがムニの部屋にプレゼントを届けに来る ---
      this.socialize();
      const givers = MAIN_IDS.filter(id => id !== 'MUNI');
      const rng = deriveRng(this.game.worldSeed, 'xmasgift', this.world.clock.day);
      const giver = givers[Math.floor(rng() * givers.length)];
      this.states.MUNI = { loc: 'HOME_MUNI', activity: 'NAP' };   // 眠っている(zzz表示)がcastには含める
      this.states[giver] = { loc: 'HOME_MUNI', activity: 'VISIT' };
    } else {
