...
import { GameClock, SEASON_LABEL, WEATHER_LABEL, SEASON_DAYS } from './core/clock.js';
...
import { createCat, createBird, createDog } from './three/animals.js';
...
// セーブ
// ============================================================
const SAVE_KEY = 'kowloon_life17_save_v1';
...
// ★敷地探索モード (STEP 5〜)
//   true  … 一人称で九龍城の敷地を歩く新方式。book (九龍書店) だけ接続済み。
//   false … STEP 0 と同じ旧タワー方式に戻る。
//   旧タワーのコードは一切削除していないので、この1行だけで往復できる。
export const SITE_MODE_ENABLED = true;

/** 敷地モードの窓口。無効時は null のまま */
let siteMode = null;
...
const game = {
  ...
  // 敷地モードでは「プレイヤーが今いる区画」を返す。
  // GROUND / book など、既存の会話条件 (location == book) がそのまま成立する値。
  currentLocation: () => (siteMode ? siteMode.getCurrentLocation() : (mode === 'focus' ? focusLocId : 'NONE')),
  ...
  currentEvent: () => eventForDay(clock.season, ((clock.day - 1) % SEASON_DAYS) + 1) ?? null,
...
// ---- 動物: 一階の外(地上)に犬がやってくる ----
const groundDog = createDog();
groundDog.position.set(3.5, 0, 8.5);
groundDog.rotation.y = -0.6;
groundBits.group.add(groundDog);
// 屋上の猫・鳥は buildTower 内で roofGroup に追加(タワー再構築のたびに作り直す)
let roofCat = null, roofBird = null;
const wanderTargets = { cat: null, bird: null, dog: null };
const wanderTimers = { cat: 0, bird: 0, dog: 0 };
function animalTarget(kind) {
  if (kind === 'cat') return new THREE.Vector3(rand(-5, 5), 0.25, rand(-3.5, 3.5));
  if (kind === 'bird') return new THREE.Vector3(rand(-5, 5), rand(0.5, 1.4), rand(-3.5, 3.5));
  return new THREE.Vector3(rand(-4, 8), 0, rand(6, 11));   // 犬(地上)
}
// 動物は昼(朝/昼/夕)にやってくる。夜は帰る(非表示)。
function updateAnimals(dt) {
  const daytime = ['MORNING', 'DAY', 'EVENING'].includes(clock.band);
  for (const [kind, mesh] of [['cat', roofCat], ['bird', roofBird], ['dog', groundDog]]) {
    if (!mesh) continue;
    mesh.visible = daytime;
    if (!daytime) continue;
    wanderTimers[kind] -= dt;
    if (!wanderTargets[kind] || wanderTimers[kind] <= 0) { wanderTargets[kind] = animalTarget(kind); wanderTimers[kind] = rand(3, 7); }
    const tgt = wanderTargets[kind];
    const dir = tgt.clone().sub(mesh.position); dir.y = 0;
    if (dir.length() > 0.12) {
      mesh.rotation.y = Math.atan2(dir.x, dir.z);
      mesh.position.addScaledVector(dir.normalize(), Math.min((kind === 'bird' ? 0.9 : 0.5) * dt, 0.06));
    }
    mesh.position.y += (tgt.y - mesh.position.y) * Math.min(1, dt * 2);
    if (kind === 'cat') { mesh.userData.tail.rotation.z = Math.sin(time * 4) * 0.3; }
    else if (kind === 'bird') { const f = Math.sin(time * 12) * 0.6; mesh.userData.wingL.rotation.z = f; mesh.userData.wingR.rotation.z = -f; mesh.position.y += Math.sin(time * 3) * 0.02; }
    else { mesh.userData.tail.rotation.y = Math.sin(time * 6) * 0.4; }
  }
}
...
function buildTower() {
  ...
  // ---- 動物: 屋上に猫と鳥がやってくる ----
  roofCat = createCat();
  roofCat.position.set(4, 0.25, 2);
  roofCat.rotation.y = -0.4;
  roofGroup.add(roofCat);
  roofBird = createBird();
  roofBird.position.set(-3, 0.55, -2);
  roofGroup.add(roofBird);
  wanderTargets.cat = null; wanderTargets.bird = null;   // 再構築時にワンダーをリセット
  ...
  // 敷地モードでは旧タワーの見た目だけ隠す。
  // locAnchors とコイン配置は他23施設のためにそのまま使うので、buildTower 自体は動かし続ける。
  if (siteMode) {
    siteMode.onTowerRebuilt([towerGroup, roofClickBox, groundClickBox, groundBits.group]);
  }
...
function updateNpcPositions(instant = false) {
  ...
  // 敷地モードでは、新方式の空間にいないNPCを隠す
  // (旧タワーの座標に置かれたままだと、空中に浮いて見えてしまう)
  if (siteMode) siteMode.updateNpcVisibility(schedule);
}
...
clock.on('day', () => {
  ...
  if (ev) {
    toast(`${ev.emoji} 今日は${ev.name}!`);
    const venue = ev.venue ?? 'hall';
    const venueReady = (venue === 'ROOF' || venue === 'GROUND') ? true : game.isBuilt(venue);
    if (venueReady && EVENT_INTRO[ev.id]) toast(EVENT_INTRO[ev.id]);
  }
  // 病気の抽選 (病院が建っていれば)
  rollSickness();
  computeThiefWindows();
...
// ---- 花火大会(屋上イベント): 九龍城の外に花火が上がる ----
let roofTopY = 0;
const fireworks = [];   // { group, parts:[{mesh,vx,vy,vz}], life, ttl }
let fwTimer = 0;
function spawnFirework() {
  const g = new THREE.Group();
  const cx = rand(-11, 11), cy = roofTopY + rand(3, 12), cz = rand(-9, 1);
  ...
  for (let i = 0; i < N; i++) {
    ...
    const sp = rand(3.5, 6.5);
    parts.push({ mesh: m, vx: Math.sin(phi) * Math.cos(theta) * sp, vy: Math.cos(phi) * sp, vz: Math.sin(phi) * Math.sin(theta) * sp });
    g.add(m);
  }
  scene.add(g);
  fireworks.push({ group: g, parts, ttl: 1.6 });
}
function updateFireworks(dt) {
  const ev = game.currentEvent?.();
  const active = ev?.id === 'HANABI' && (ev.bands ?? []).includes(clock.band);
  if (active) {
    fwTimer -= dt;
    if (fwTimer <= 0 && fireworks.length < 6) { spawnFirework(); fwTimer = rand(0.5, 1.1); }
  }
  for (let i = fireworks.length - 1; i >= 0; i--) {
    const fw = fireworks[i];
    fw.ttl -= dt;
    const k = Math.max(0, fw.ttl / 1.6);
    ...
    if (fw.ttl <= 0) {
      fw.group.traverse(o => { if (o.isMesh) { o.geometry.dispose(); o.material.dispose(); } });
...
function focusLoc(locId) {
  if (siteMode) return;   // 敷地モードでは歩いて移動するので、寄りのカメラは使わない
...
function startDialogue(charId) {
  ...
  if (!node) return;
  // 敷地モードでは会話中の移動と視点を止め、ポインタロックを外して
  // 既存のDOM会話パネル(クリックで進む)がそのまま使えるようにする
  siteMode?.setControlEnabled(false);
...
function closeDialogue(silent = false) {
  ...
  siteMode?.setControlEnabled(true);   // 一人称操作へ復帰 (再ロックはクリック待ち)
...
function npcVisibleToPlayer(charId) {
  ...
  if (siteMode) return siteMode.isLocVisible(loc);
...
function handleClick(ev) {
  ...
  // 敷地モードではクリック = ポインタロック開始。対象の選択は視線とEキーで行う
  if (siteMode) { siteMode.onCanvasClick(); return; }
  ...
  setTimeout(() => {
    ...
    if (drop.kind === 'facility') {
      ...
      if (dup) {
        ...
        state.unlockedFacilities.push(drop.id);   // ← 既存処理。新しいフラグは作らない
        extra = '「増築」から圓を払って建てられる!';
        // 敷地モードで接続済みの施設なら、区画を locked → unlocked へ差分更新する
        if (siteMode?.isMigrated(drop.id)) {
          siteMode.syncFacilities();
          extra = `敷地の入口が「近日開店」に変わった! (${fdef.cost}圓で開店)`;
        }
        sfx('fanfare');
      }
    } else {
...
function buildFacility(fid) {
  ...
  // --- 既存のデータ更新。形式は一切変えない (isBuilt / built() の根拠) ---
  state.yen -= fdef.cost;
  ...
  roomLight.intensity = 0;
  el('focus-bar').style.display = 'none';
  buildTower();   // locAnchors とコイン配置のため、敷地モードでも動かす
  updateBadges();
  updateHUD();
  sfx('build');

  if (siteMode?.isMigrated(fid)) {
    // 敷地モード: 区画を unlocked → opened へ差分更新 (シャッターが開き、看板が点灯する)
    siteMode.syncFacilities();
    toast(`🏬 ${fdef.name} が開店した! 中に入れるようになった`);
  } else {
...
function animate() {
  ...
  // カメラ: 敷地モードは一人称、旧方式は OrbitControls + tween
  if (siteMode) {
    siteMode.update(dt, schedule);
  } else {
    if (tween.active) {
  ...
  const ambResult = ambient.tick(dt, game.currentLocation(), game);
...
// 敷地モードの初期化は buildTower() より前に行う。
// buildTower() の中から siteMode.onTowerRebuilt() を呼ぶため。
if (SITE_MODE_ENABLED) {
  try {
    const m = await import('./three/siteMode.js?v=step7');
    siteMode = await m.createSiteMode({
      scene, camera, renderer, game, locAnchors, npcs, state, clock,
      chars: CHARS, allIds: ALL_IDS, testMode: TEST_MODE,
      actions: {
        startDialogue,
        buildFacility,
        toast,
        sfx,
        isBuilt: game.isBuilt,
        refreshNpcs: updateNpcPositions,
        // コインは既存の respawnCoins() が旧タワーの店内に配置する。
        // 敷地モードでは同じ配列をそのまま借りて、3D店内へ置き直して拾う。
        // 報酬量・出現数・ガチャ価格には一切触れていない。
        coins,
        collectCoin,
      },
    });
    // 旧タワー用の補助ライトは敷地モードでは不要 (ライト数を増やさないため)
    roomLight.visible = false;
    warmBack.visible = false;
    // OrbitControls は enabled=false でもポインタイベントを掴み続け、
    // ポインタロック中に setPointerCapture が InvalidStateError を投げる。
    // リスナーごと外す (旧タワーへ戻すときは再読み込みで再構築される)。
    controls.enabled = false;
    controls.dispose();
    console.info('[game] 敷地探索モードで起動しました (SITE_MODE_ENABLED=true)');
  } catch (e) {
    console.error('[game] 敷地モードの初期化に失敗。旧タワー方式で続行します:', e);
    siteMode = null;
  }
}
...
if (siteMode) {
  el('hint').textContent =
    'クリックで開始 / W・↑ 前進 / ↓ 後退 / ← → 左右 / マウス 視点 / Shift ダッシュ / E 調べる・話す / Esc カーソル';
  el('focus-bar').style.display = 'none';
} else {
...
window.__kl = {
  ...
  updateEventObject, openWishes, openShop, computeThiefWindows, rollSickness, respawnCoins,
  coins, npcs, camera,
  SITE_MODE_ENABLED,
  __locAnchors: locAnchors,
  get siteMode() { return siteMode; },
