...
const INTERIORS = {
  ...
  // 集会所: がらんとした畳敷きの大広間(季節イベント会場)。中央に飾り台。
  hall(g) {
    g.add(makeShell(0xe8dcc0, 0x9aa860));
    // 畳の目
    for (let i = -3; i <= 3; i++) g.add(box(0.04, 0.052, FLOOR_D - 1, 0x3a3a2a, { x: i * 1.6, y: 0.027 }));
    // 掲示板
    const board = signBoard('九龍集会所', { w: 3.2, h: 0.7, fg: '#5a3a1a', bg: '#f0e4c0', glow: '#ffd24a' });
    board.position.set(0, 3.0, -4.5); g.add(board);
    // 座布団を円に並べる(集まって話す場)
    const cushCols = [0xd84a4a, 0x4a6ad8, 0x4ac86a, 0xe8c84a, 0xd05ac8, 0x5ad0d8, 0xe8883a];
    for (let i = 0; i < 7; i++) {
      const a = (i / 7) * Math.PI * 2;
      g.add(box(0.7, 0.1, 0.7, cushCols[i], { x: Math.cos(a) * 2.6, y: 0.06, z: Math.sin(a) * 2.0 + 0.3, ry: a }));
    }
    // 中央の飾り台(イベントオブジェクトはゲーム側でここに置く)
    g.add(cyl(0.6, 0.7, 0.15, 0x8a6a3a, { x: 0, y: 0.08, z: 0.3, seg: 8 }));
    g.userData.eventAnchor = { x: 0, y: 0.16, z: 0.3 };
    // 提灯
    for (const x of [-3.5, 0, 3.5]) g.add(grp(cyl(0.2, 0.2, 0.36, 0xd83a2a, { emissive: 0xff5a2a, emissiveIntensity: 0.7 }), cyl(0.09, 0.09, 0.05, 0x2a2a2a, { y: 0.22 })).translateX(x).translateY(FLOOR_H - 0.5).translateZ(-3.5));
    return [[5.6, 0.3, -3.8], [-5.6, 0.3, 3.5], [4.5, 0.3, 3.5], [-4.5, 0.3, -3.8]];
  },

  // ケーキ屋: ショーケースにケーキ、パステルな内装
  cake(g) {
    g.add(makeShell(0xffe8f0, 0xe8c0d0));
    // ショーケース
    g.add(grp(
      box(4.5, 1.0, 0.9, 0xffffff, { y: 0.5 }),
      box(4.4, 0.7, 0.5, 0xd0f0ff, { y: 1.2, z: 0.1, transparent: true, opacity: 0.4, emissive: 0xa8d8f0, emissiveIntensity: 0.3 }),
    ).translateX(-1.5).translateZ(-3.5));
    // ケーキたち
    const cakeCols = [0xff8ab0, 0xffd88a, 0x8ad0a0, 0xd8a8f0, 0xffb0a0];
    for (let i = 0; i < 6; i++) {
      const x = -3.5 + i * 0.7;
      g.add(cyl(0.22, 0.24, 0.16, pick([0xf0e0c0, 0x8a5a3a]), { x, y: 1.0, z: -3.4, seg: 14 }));
      g.add(sph(0.05, pick(cakeCols), { x, y: 1.12, z: -3.4 }));
    }
    // ホールケーキ台
    g.add(grp(cyl(0.5, 0.55, 0.06, 0xffffff, { y: 0.5 }), cyl(0.4, 0.42, 0.3, 0xffe0d0, { y: 0.68 }), sph(0.08, 0xff5a7a, { y: 0.88 })).translateX(2.5).translateZ(-3.8));
    // カフェテーブル
    for (const [x, z] of [[2.5, -0.5], [4.5, 1.5]]) g.add(grp(cyl(0.4, 0.4, 0.05, 0xffffff, { y: 0.72 }), cyl(0.05, 0.06, 0.7, 0xe8b0c8, { y: 0.36 })).translateX(x).translateZ(z));
    const sign = signBoard('ケーキ屋スゥレ', { w: 3.0, h: 0.6, fg: '#fff', bg: '#c85a8a', glow: '#ff9ac8' });
    sign.position.set(0, 3.2, -4.5); g.add(sign);
    g.add(pendantLight(0xffe0f0).translateX(-1.5).translateZ(-3.5), pendantLight(0xffe0f0).translateX(3).translateZ(0.5));
    return [[5.6, 0.3, -3.5], [-5.6, 0.3, 2.0], [-1.5, 1.1, -3.5], [4.5, 0.78, 1.5]];
  },

  // 病院: 白い待合、ベッド、緑十字
  hospital(g) {
    g.add(makeShell(0xf0f8f8, 0xd8e4e4));
    // 受付カウンター
    const c = counter(3.5, 0xd8e8e8, 0xeef4f4); c.position.set(-3, 0, -3.5); g.add(c);
    g.add(box(0.4, 0.5, 0.3, 0xffffff, { x: -3.5, y: 1.3, z: -3.5 }));
    // 診察ベッド ×2
    for (const x of [3, 5]) {
      g.add(grp(box(1.0, 0.5, 2.0, 0xe8eef0, { y: 0.5 }), box(1.0, 0.1, 2.0, 0xa8d0e0, { y: 0.78 }), box(1.0, 0.08, 0.4, 0xffffff, { y: 0.86, z: -0.7 })).translateX(x).translateZ(-3.0));
      g.add(box(1.6, 2.2, 0.04, 0xd0e8e8, { x: x - 0.9, y: 1.1, z: -3.0, transparent: true, opacity: 0.5 })); // カーテン
    }
    // 待合の椅子
    for (let i = 0; i < 4; i++) g.add(box(0.5, 0.5, 0.5, 0x8ab0b8, { x: -5 + i * 0.6, y: 0.25, z: 3.5 }));
    // 緑十字
    g.add(box(0.9, 0.3, 0.08, 0x2ab84a, { y: 3.0, z: -4.5, emissive: 0x2ab84a, emissiveIntensity: 0.9 }));
    g.add(box(0.3, 0.9, 0.08, 0x2ab84a, { y: 3.0, z: -4.5, emissive: 0x2ab84a, emissiveIntensity: 0.9 }));
    // 観葉植物・点滴スタンド
    g.add(grp(cyl(0.02, 0.02, 1.6, 0xc8c8d0, { y: 0.8 }), box(0.16, 0.24, 0.06, 0xd0e8f0, { y: 1.5, transparent: true, opacity: 0.6 })).translateX(4).translateZ(-1.0));
    const sign = signBoard('九龍総合病院', { w: 3.0, h: 0.6, fg: '#fff', bg: '#2a7a5a', glow: '#5affd0' });
    sign.position.set(0, 3.4, -4.5); g.add(sign);
    return [[5.6, 0.3, -3.5], [-5.6, 0.3, 3.5], [-3, 0.3, -2.5], [3.5, 0.3, 3.5]];
  },

  // ミニ水族館: 大水槽、青い光
  aquarium(g) {
    g.add(makeShell(0x14324a, 0x0e2436));
    // 壁一面の大水槽
    const tank = grp(
      box(9, 2.6, 0.5, 0x1a4a6a, { y: 1.6, z: -4.4 }),
      box(8.4, 2.2, 0.1, 0x2a88c8, { y: 1.6, z: -4.1, transparent: true, opacity: 0.55, emissive: 0x2a88c8, emissiveIntensity: 0.5 }),
    );
    g.add(tank);
    // 魚のシルエット
    const fishCols = [0xff8a4a, 0xffd84a, 0xd0d0e0, 0x4affd0, 0xff5a8a];
    for (let i = 0; i < 10; i++) {
      g.add(box(rand(0.18, 0.34), rand(0.1, 0.18), 0.04, pick(fishCols), { x: rand(-4, 4), y: rand(0.7, 2.6), z: -4.0, emissive: 0x224466, emissiveIntensity: 0.3 }));
    }
    // クラゲ(丸い発光体)
    for (let i = 0; i < 4; i++) g.add(sph(0.18, 0xd0a8f0, { x: rand(-3.5, 3.5), y: rand(1.0, 2.4), z: -3.9, transparent: true, opacity: 0.6, emissive: 0xa06ae0, emissiveIntensity: 0.6 }));
    // 円柱水槽(中央)
    g.add(grp(cyl(0.7, 0.7, 2.4, 0x2a88c8, { y: 1.4, transparent: true, opacity: 0.4, emissive: 0x2a88c8, emissiveIntensity: 0.5, seg: 20 }), cyl(0.75, 0.8, 0.2, 0x1a3a4a, { y: 0.1, seg: 20 })).translateX(0).translateZ(1.5));
    // ベンチ
    g.add(box(2.4, 0.4, 0.6, 0x2a4a5a, { x: -3.5, y: 0.25, z: 2.5 }));
    g.add(box(2.4, 0.4, 0.6, 0x2a4a5a, { x: 3.5, y: 0.25, z: 2.5 }));
    const sign = signBoard('ミニ水族館', { w: 3.0, h: 0.6, fg: '#cde8ff', bg: '#0a2438', glow: '#4ab8ff' });
    sign.position.set(0, 3.3, -4.55); g.add(sign);
    return [[5.6, 0.3, 2.5], [-5.6, 0.3, 2.5], [0, 0.3, 3.0], [-3.5, 0.55, 2.5]];
  },

  // 魚屋: 発泡スチロールの箱に氷と魚、まな板
  fishshop(g) {
    g.add(makeShell(0xdfeef0, 0x9ab0b8));
    // 陳列台(氷+魚)
    for (const [x, z] of [[-3, -3.5], [0, -3.5], [3, -3.5]]) {
      g.add(grp(
        box(2.4, 0.7, 1.0, 0x6a8a9a, { y: 0.35 }),
        box(2.3, 0.1, 0.9, 0xe8f4f8, { y: 0.72 }),   // 氷
        box(0.5, 0.12, 0.24, 0x9ab8c8, { x: -0.5, y: 0.82, ry: 0.3 }),  // 魚
        box(0.5, 0.12, 0.24, 0xc89a6a, { x: 0.3, y: 0.82, ry: -0.2 }),
        sph(0.14, 0xd85a5a, { x: 0.6, y: 0.82 }),   // タコ/マグロ
      ).translateX(x).translateZ(z));
    }
    // まな板と包丁(ネオの実演台)
    g.add(grp(
      box(1.6, 0.9, 0.8, 0x8a6a4a, { y: 0.45 }),
      box(1.2, 0.06, 0.6, 0xd8c8a8, { y: 0.93 }),        // まな板
      box(0.4, 0.02, 0.1, 0xc8c8d0, { x: 0.3, y: 0.97, metalness: 0.7 }),  // 包丁
      box(0.4, 0.1, 0.16, 0x9ab8c8, { x: -0.2, y: 0.98 }), // さばく魚
    ).translateX(-3.5).translateZ(1.5));
    // 値札の旗
    for (const x of [-3, 0, 3]) g.add(box(0.5, 0.3, 0.02, 0xffe45a, { x, y: 1.3, z: -3.5 }));
    const sign = signBoard('魚屋 龍鱗', { w: 2.6, h: 0.6, fg: '#eaffff', bg: '#1a5a5a', glow: '#4affe0' });
    sign.position.set(1, 3.2, -4.5); g.add(sign);
    return [[5.6, 0.3, -3.0], [-5.6, 0.3, 3.5], [0, 0.85, -3.5], [3.5, 0.3, 2.0]];
  },

  // 焼肉屋: 各テーブルに七輪、煙、赤い内装
  yakiniku(g) {
    g.add(makeShell(0x6a3a2a, 0x3a2018));
    // テーブル(七輪付き) ×3
    for (const [x, z] of [[-3, -2], [3, -2], [0, 2]]) {
      g.add(grp(
        cyl(0.7, 0.7, 0.06, 0x2a2a30, { y: 0.72, seg: 8 }),
        cyl(0.06, 0.08, 0.7, 0x1a1a20, { y: 0.36 }),
        cyl(0.28, 0.3, 0.2, 0x33333a, { y: 0.86 }),         // 七輪
        cyl(0.24, 0.24, 0.05, 0xff5a2a, { y: 0.96, emissive: 0xff4a1a, emissiveIntensity: 1.2 }),  // 炭火
        box(0.16, 0.12, 0.16, 0xd85a4a, { x: 0.1, y: 1.0 }), // 肉
        box(0.16, 0.12, 0.16, 0xd85a4a, { x: -0.1, y: 1.0, z: 0.08 }),
      ).translateX(x).translateZ(z));
      // 座席
      for (const [dx, dz] of [[-0.7, 0], [0.7, 0]]) g.add(cyl(0.22, 0.22, 0.4, 0x8a2a2a, { x: x + dx, y: 0.2, z: z + dz }));
      // 煙
      g.add(sph(0.14, 0xdddddd, { x, y: 1.4, z, transparent: true, opacity: 0.3 }));
      g.add(sph(0.1, 0xdddddd, { x: x + 0.1, y: 1.7, z, transparent: true, opacity: 0.2 }));
    }
    // 換気ダクト
    for (const [x, z] of [[-3, -2], [3, -2], [0, 2]]) g.add(cyl(0.3, 0.3, 0.6, 0x555560, { x, y: FLOOR_H - 0.4, z, metalness: 0.5 }));
    const sign = signBoard('焼肉ドラゴン', { w: 2.8, h: 0.7, fg: '#ffe0a0', bg: '#4a1008', glow: '#ff5a3a' });
    sign.position.set(0, 3.2, -4.5); g.add(sign);
    return [[5.6, 0.3, -3.5], [-5.6, 0.3, 3.5], [3, 0.3, 3.5], [-3.5, 0.3, -3.8]];
  },

  // 教会(九龍聖堂): 祭壇・十字架・ステンドグラス・懺悔室
  church(g) {
    g.add(makeShell(0xe8e2d2, 0x8a7a62));
    // 奥の祭壇と大十字架
    g.add(box(3.2, 0.6, 1.0, 0xd8ccb0, { y: 0.3, z: -4.0 }));            // 祭壇
    g.add(box(2.0, 0.1, 0.6, 0xf4eede, { y: 0.65, z: -4.0 }));
    g.add(box(0.28, 2.0, 0.14, 0xc8a86a, { y: 2.3, z: -4.5, metalness: 0.4 }));   // 十字(縦)
    g.add(box(1.0, 0.28, 0.14, 0xc8a86a, { y: 2.6, z: -4.5, metalness: 0.4 }));   // 十字(横)
    // ステンドグラス(奥壁)
    const glassCols = [0xd84a5a, 0x4a6ad8, 0x4ac86a, 0xe8c84a, 0xd05af0];
    for (let i = 0; i < 5; i++) g.add(box(0.7, 1.4, 0.05, glassCols[i], { x: -3 + i * 1.5, y: 2.6, z: -4.55, emissive: glassCols[i], emissiveIntensity: 0.5, transparent: true, opacity: 0.85 }));
    // 会衆席(ベンチ)×左右2列
    for (let r = 0; r < 3; r++) {
      for (const x of [-2.2, 2.2]) {
        g.add(grp(box(1.8, 0.1, 0.5, 0x8a6a4a, { y: 0.42 }), box(1.8, 0.5, 0.1, 0x8a6a4a, { y: 0.65, z: -0.2 })).translateX(x).translateZ(-1.5 + r * 1.4));
      }
    }
    // 中央通路のろうそく
    for (const z of [-2.5, 0, 2]) {
      g.add(cyl(0.05, 0.06, 0.3, 0xf0ead0, { x: 0, y: 0.9, z }));
      g.add(sph(0.05, 0xffb84d, { x: 0, y: 1.08, z, emissive: 0xff9a2a, emissiveIntensity: 1.4, sy: 1.6 }));
      g.add(cyl(0.02, 0.02, 0.75, 0x9a8a6a, { x: 0, y: 0.4, z }));
    }
    // 懺悔室(右手前): 木の小部屋+格子窓
    const booth = grp(
      box(1.6, 2.6, 1.6, 0x5a4632, { y: 1.3 }),
      box(1.4, 2.2, 0.06, 0x3a2c1c, { y: 1.1, z: 0.8 }),          // 前面
      box(0.6, 1.0, 0.02, 0x2a1e12, { x: -0.4, y: 1.4, z: 0.83 }),  // 参列者側の扉
      // 格子窓
      box(0.4, 0.4, 0.04, 0x1a120a, { x: 0.35, y: 1.5, z: 0.83 }),
      box(0.42, 0.04, 0.05, 0xc8a86a, { x: 0.35, y: 1.6, z: 0.85 }),
      box(0.42, 0.04, 0.05, 0xc8a86a, { x: 0.35, y: 1.4, z: 0.85 }),
      box(0.04, 0.42, 0.05, 0xc8a86a, { x: 0.25, y: 1.5, z: 0.85 }),
      box(0.04, 0.42, 0.05, 0xc8a86a, { x: 0.45, y: 1.5, z: 0.85 }),
      // 小さな十字
      box(0.06, 0.3, 0.03, 0xc8a86a, { y: 2.7, z: 0.8 }),
      box(0.2, 0.06, 0.03, 0xc8a86a, { y: 2.76, z: 0.8 }),
    );
    booth.position.set(4.2, 0, 2.5);
    g.add(booth);
    // 天井のシャンデリア風
    g.add(grp(cyl(0.02, 0.02, 0.5, 0x8a7a5a, { y: FLOOR_H - 0.25 }), cyl(0.35, 0.4, 0.12, 0xc8a86a, { y: FLOOR_H - 0.55 }), sph(0.1, 0xffe8b0, { y: FLOOR_H - 0.6, emissive: 0xffd890, emissiveIntensity: 0.9 })).translateZ(-1));
    const sign = signBoard('九龍聖堂', { w: 2.6, h: 0.6, fg: '#5a4a2a', bg: '#f0e8d0', glow: '#e8e0c8' });
    sign.position.set(-1, 3.3, -4.5); g.add(sign);
    // 懺悔室が主なNPC位置(神父側)
    return [[4.2, 0.3, 2.5], [-5.6, 0.3, 3.5], [0, 0.3, -3.0], [-2.2, 0.55, 0]];
  },

  cinema(g) {
    g.add(makeShell(0x2a2432, 0x3a2a2a));   // 暗い映画館
    // 大スクリーン(奥壁いっぱい・発光)
    g.add(box(9.5, 4.6, 0.2, 0x0a0a12, { y: 2.3, z: -4.5 }));                       // 額縁
    g.add(box(8.8, 4.0, 0.06, 0xdfe6f0, { y: 2.3, z: -4.38, emissive: 0x9ab0d8, emissiveIntensity: 0.7 }));  // 映写面
    g.add(box(8.8, 0.5, 0.02, 0x2a3a5a, { y: 3.4, z: -4.34, emissive: 0x3a5a8a, emissiveIntensity: 0.5 }));   // 映像の帯
    // 赤いカーテン(左右)
    for (const x of [-4.4, 4.4]) g.add(box(0.7, 4.6, 0.5, 0x8a1f2a, { x, y: 2.3, z: -4.1, roughness: 1 }));
    // 客席(赤いシート)3列×左右
    for (let r = 0; r < 3; r++) {
      for (const x of [-2.4, 0, 2.4]) {
        g.add(grp(
          box(0.9, 0.5, 0.7, 0xa8283a, { y: 0.45 }),          // 座面+背
          box(0.9, 0.7, 0.15, 0xb83545, { y: 0.75, z: -0.28 }),
        ).translateX(x).translateZ(0.5 + r * 1.5));
      }
    }
    // 赤絨毯の通路
    g.add(box(1.2, 0.02, 8, 0x6a1520, { y: 0.01, z: 0 }));
    // 映写窓(手前上)
    g.add(box(1.2, 0.6, 0.1, 0x101018, { y: 3.4, z: 4.4 }));
    g.add(box(0.5, 0.3, 0.06, 0xffe8a0, { y: 3.4, z: 4.36, emissive: 0xffd070, emissiveIntensity: 1.2 }));
    // ポスター(側壁)
    for (const [zz, col] of [[-1.5, 0x3a5ad8], [1.5, 0xd83a6a]]) g.add(box(0.06, 1.6, 1.0, col, { x: -6.7, y: 1.9, z: zz, emissive: col, emissiveIntensity: 0.3 }));
    const sign = signBoard('シネマ・ホシカゲ', { w: 3.2, h: 0.6, fg: '#1a1208', bg: '#ffcf4a', glow: '#ffcf4a' });
    sign.position.set(0, 3.6, 4.3); g.add(sign);
    // 館長の立ち位置は通路脇
    return [[5.6, 0.3, 2.5], [-5.6, 0.3, 2.5], [3.2, 0.75, 1.0], [-3.2, 0.3, 3.5]];
  },

  donut(g) {
    g.add(makeShell(0xfff0e0, 0xd8a878));
    // ショーケース(カウンター)
    const c = counter(5, 0xc87a4a, 0xf0d8b0); c.position.set(-1, 0, -2.5); g.add(c);
    g.add(box(5, 0.9, 0.1, 0xbfe4ec, { x: -1, y: 1.4, z: -2.15, transparent: true, opacity: 0.35 }));   // ガラス
    // ドーナツをトレイに並べる(色とりどりのトーラス)
    const dcols = [0xf0a0c0, 0x8a5a3a, 0xf0d84a, 0xd0f0a0, 0xf0b070, 0xd8a0f0];
    for (let i = 0; i < 12; i++) {
      const d = new THREE.Mesh(new THREE.TorusGeometry(0.16, 0.075, 8, 14), new THREE.MeshStandardMaterial({ color: dcols[i % dcols.length], roughness: 0.8 }));
      d.rotation.x = Math.PI / 2;
      d.position.set(-3 + (i % 6) * 0.7, 1.02, -2.5 + Math.floor(i / 6) * 0.4);
      g.add(d);
    }
    // 壁のメニュー
    g.add(box(2.6, 1.4, 0.06, 0x6a4a32, { x: 3.5, y: 2.2, z: -4.4 }));
    g.add(box(2.3, 1.1, 0.02, 0xfff4e8, { x: 3.5, y: 2.2, z: -4.36 }));
    // 大きな看板ドーナツ(飾り)
    const bigD = new THREE.Mesh(new THREE.TorusGeometry(0.6, 0.28, 10, 20), new THREE.MeshStandardMaterial({ color: 0xf0a0c0, roughness: 0.7, emissive: 0x5a2a3a, emissiveIntensity: 0.2 }));
    bigD.position.set(4.6, 2.6, -3.8); g.add(bigD);
    // 丸テーブル+スツール
    for (const [x, z] of [[2.5, 1.5], [-3.5, 1.5]]) {
      g.add(cyl(0.5, 0.5, 0.06, 0xe8c8a0, { x, y: 0.9, z }));
      g.add(cyl(0.08, 0.08, 0.9, 0x8a6a4a, { x, y: 0.45, z }));
      for (const a of [0, Math.PI]) g.add(cyl(0.22, 0.22, 0.08, 0xd88a5a, { x: x + Math.cos(a) * 0.85, y: 0.55, z: z + Math.sin(a) * 0.85 }));
    }
    g.add(pendantLight(0xffd8a0).translateX(-1).translateZ(-2.5), pendantLight(0xffd8a0).translateX(2.5).translateZ(1.5));
    const sign = signBoard('ドーナツ九龍', { w: 2.8, h: 0.6, fg: '#5a2a10', bg: '#ffcf9a', glow: '#ffb07a' });
    sign.position.set(-1, 3.2, -4.5); g.add(sign);
    return [[5.6, 0.3, -3.0], [-5.6, 0.3, 2.5], [-1, 0.75, -2.0], [2.5, 0.3, 3.2]];
  },
};
