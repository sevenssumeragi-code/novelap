...
export const DLG_V8 = [
  ...
  T('DLG_HOSHIKAGE_FIRST', 'HOSHIKAGE', {
    ...
    lines: [
      { sp: 'HOSHIKAGE', text: 'よう来なさった! シネマ・ホシカゲへようこそ! 館長の星影幸太郎じゃ、よろしくのう。' },
      { sp: 'HOSHIKAGE', text: 'この街に映画館を建ててくれるとは、おぬし、見どころがあるのう! 映画は人生の全てじゃ。異論は認めるが、わしは曲げんぞ!' },
      { sp: 'HOSHIKAGE', text: 'さあ、今日は何を観るかのう? ……おっと、語り出す前に。まずは一本、暗闇に身を委ねてみるがよい。' },
    ],
    effects: ['flag(FLG_MET_HOSHIKAGE)', 'aff(HOSHIKAGE,2)', 'log(EVENT)'],
  }),
  T('DLG_HOSHIKAGE_TALK_1', 'HOSHIKAGE', { pool: 'COND', cond: 'location == cinema', weight: 10, lines: [{ sp: 'HOSHIKAGE', text: '今週の特集はのう、雨の似合う映画ばかり集めたんじゃ。……レニィ君が気に入ってくれてのう、途中で寝ておったが。' }], effects: ['aff(HOSHIKAGE,1)'] }),
  T('DLG_HOSHIKAGE_TALK_2', 'HOSHIKAGE', { pool: 'COND', cond: 'location == cinema and weather == RAIN', weight: 12, lines: [{ sp: 'HOSHIKAGE', text: '雨の日の映画館は、格別でのう。傘を畳んで暗闇に入る、あの背徳感。分かるかの?' }], effects: ['aff(HOSHIKAGE,1)'] }),
  T('DLG_HOSHIKAGE_TALK_3', 'HOSHIKAGE', { lines: [{ sp: 'HOSHIKAGE', text: '床屋の親父とは西部劇、郵便屋君とは最新作。……映画仲間がおるというのは、ええもんじゃのう。' }], effects: ['aff(HOSHIKAGE,1)'] }),

  // 星影 × 主要人物
  A('DLG_AMB_HOSHI_LENNY', ['HOSHIKAGE', 'LENNY'], { cond: 'location == cinema', weight: 16, lines: [
    { sp: 'HOSHIKAGE', text: 'レニィ君、また最前列で寝ておるのう。……ええんじゃよ、映画館は世界一贅沢な昼寝の場所じゃ。' },
    { sp: 'LENNY', text: 'えへへ……星影さんの映画館、暗くてあったかくて、ぼくの特等席なんだ。' },
    { sp: 'HOSHIKAGE', text: 'ほっほ、そう言われると悪い気はせんのう。今度、子守唄みたいな映画を用意しておくわい。' },
  ...
  A('DLG_AMB_HOSHI_HYU', ['HOSHIKAGE', 'HYU'], { cond: 'location == cinema', weight: 16, lines: [
    ...
    { sp: 'HOSHIKAGE', text: 'ほっほっほ、おぬしは自分自身が主演の映画を生きておるのう! そういう客、わしは嫌いじゃないぞ。' },
  ...
  A('DLG_AMB_HOSHI_JIN', ['HOSHIKAGE', 'JIN'], { cond: 'location == cinema', weight: 16, lines: [
    ...
    { sp: 'HOSHIKAGE', text: 'ジンパチ君にはこれじゃ、拳で全てを語る名作! ……観たあと必ず筋トレしたくなるから、覚悟しておくがよい。' },
  ...
  A('DLG_AMB_HOSHI_GERU', ['HOSHIKAGE', 'GERU'], { cond: 'location == cinema', weight: 16, lines: [
    ...
    { sp: 'HOSHIKAGE', text: 'おおっ、そこに気づくとはのう! いや実は監督にも意図があっての……よし、朝まで語ろうかの、ゲル君!' },
  ...
  A('DLG_AMB_HOSHI_MUNI', ['HOSHIKAGE', 'MUNI'], { cond: 'location == cinema', weight: 16, lines: [
    ...
    { sp: 'HOSHIKAGE', text: 'よしきた、ムニ君! 特別に一番ええ席を用意しようぞ。ポップコーンはこぼさんようにのう。' },
  ...
  A('DLG_AMB_HOSHI_NEO', ['HOSHIKAGE', 'NEO'], { cond: 'location == cinema', weight: 16, lines: [
    ...
    { sp: 'HOSHIKAGE', text: 'あるともよ、ネオ君! 騎士が誇りのために剣を取る名作をのう。おぬしなら泣くじゃろう、きっと。' },
  ...
  A('DLG_AMB_HOSHI_BARBER', ['HOSHIKAGE', 'BARBER'], { cond: 'location == cinema', weight: 15, lines: [
    ...
    { sp: 'HOSHIKAGE', text: 'ええのう親父さん! あの決闘シーン、何度観ても痺れるわい。……酒とスルメ、用意しておくぞ。' },
  ...
  A('DLG_AMB_HOSHI_POSTMAN', ['HOSHIKAGE', 'POSTMAN'], { cond: 'location == cinema', weight: 15, lines: [
    ...
    { sp: 'HOSHIKAGE', text: 'おぬしは仕事より映画が早いのう! ほっほ、入っておるよ。配達が終わったら、こっそり試写といこうかの。' },
