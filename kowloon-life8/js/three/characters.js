...
function makeHair(style, color) {
  ...
  switch (style) {
    ...
    case 'ponytail': {   // 看板娘サチ: ポニーテール
      g.add(sph(0.29, color, { y: 0.05 }));
      g.add(sph(0.1, color, { y: 0.06, z: 0.24, sx: 2.2, sy: 0.8, sz: 0.5 }));   // 前髪
      const tail = cyl(0.09, 0.03, 0.5, color, { y: -0.1, z: -0.26 });
      tail.rotation.x = -0.5;
      g.add(tail);
      g.add(cyl(0.06, 0.06, 0.05, 0xff8ac0, { y: 0.14, z: -0.24 }));   // ヘアゴム
      break;
    }
    case 'priest': {   // マルタン神父: 短い黒髪(なでつけ)
      g.add(sph(0.29, color, { y: 0.05 }));
      g.add(sph(0.1, color, { y: 0.1, z: 0.22, sx: 2.2, sy: 0.6, sz: 0.5 }));   // 前髪(七三)
      break;
    }
    case 'beanie': {   // コソ泥: ニット帽
      g.add(sph(0.28, 0x1a1a22, { y: 0.08, sy: 0.9 }));
      g.add(cyl(0.29, 0.29, 0.06, 0x2a2a34, { y: 0.02 }));   // 帽子の縁
      break;
    }
    case 'slick': {   // 星影館長: オールバックの黒髪
      g.add(sph(0.29, color, { y: 0.05, sz: 1.05 }));
      g.add(box(0.4, 0.14, 0.36, color, { y: 0.16, z: -0.04 }));   // 後ろへ流した髪
      break;
    }
  }
  return g;
...
export function createCharacterMesh(def) {
  ...
  // カフェ店員/駄菓子屋のエプロン
  if (def.apron || def.id === 'CAFEGIRL') {
    const apronCol = def.id === 'DAGASHIYA' ? 0xe8dcc0 : 0xffffff;
    g.add(box(0.36, 0.34, 0.04, apronCol, { y: 0.42, z: 0.135 }));
    g.add(box(0.36, 0.02, 0.04, def.id === 'DAGASHIYA' ? 0x9a7a5a : 0xff8ac0, { y: 0.58, z: 0.14 }));
  }

  // 神父の白い祭服(足元まで届くカソック)+ 胸の十字架
  if (def.priest) {
    g.add(box(0.5, 0.62, 0.34, def.cloth, { y: 0.31 }));                 // 裾の長い白服
    g.add(box(0.1, 0.05, 0.14, 0x8a3040, { y: 0.62, z: 0.14 }));          // 襟の赤
    g.add(box(0.05, 0.16, 0.03, 0xc8a030, { y: 0.5, z: 0.18 }));          // 胸の十字(縦)
    g.add(box(0.1, 0.05, 0.03, 0xc8a030, { y: 0.53, z: 0.18 }));          // 胸の十字(横)
  }

  // 映画館の館長 星影: 蝶ネクタイ + 深紫のジャケット襟
  if (def.director) {
    g.add(box(0.14, 0.06, 0.04, 0x8a1f2f, { y: 0.66, z: 0.14 }));          // 蝶ネクタイ
    g.add(box(0.04, 0.04, 0.05, 0xc8a030, { y: 0.66, z: 0.15 }));          // 中央の留め
    g.add(box(0.46, 0.2, 0.02, 0x2a2036, { y: 0.6, z: 0.135 }));           // ジャケットの襟
  }
  ...
  head.add(hair);
  // コソ泥の覆面(口元を隠す)
  if (def.thief) head.add(box(0.4, 0.16, 0.16, 0x33333c, { y: -0.11, z: 0.16 }));
  g.add(head);
