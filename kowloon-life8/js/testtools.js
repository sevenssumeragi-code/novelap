...
export function installTestTools(kl) {
  ...
  // ---- セクション: 季節イベント ----
  html += '<h4>🎉 季節イベントの日にする</h4><div class="tt-note">集会所が必要。時間を昼にして集合させます。</div><div class="tt-lv">';
  for (const e of Object.values(EVENTS)) html += `<button data-ev="${e.id}">${e.emoji}${e.name}</button>`;
  html += '</div>';

  // ---- セクション: 病気 / 恋人 ----
  html += '<h4>🏥 その他テスト</h4><div class="tt-row">' +
    '<button data-act="sick">誰か病気に(病院)</button>' +
    '<button data-act="loverreset">恋人をリセット</button></div>';
  ...
  panel.addEventListener('click', (ev) => {
    ...
    if (t.dataset.ev) {
      const e = EVENTS[t.dataset.ev];
      kl.clock.day = SEASON_INDEX[e.season] * 8 + e.dayOfSeason;
      kl.clock.minute = 780;   // 13:00 (集合時間帯)
      kl.updateEventObject();
      kl.updateNpcPositions(true);
      kl.updateHUD();
      refresh();
      kl.toast?.(`${e.emoji} ${e.name}の日にした`);
      if (!kl.game.isBuilt('hall')) kl.toast?.('※ 集会所を建てるとみんなが集合します');
      return;
    }
    switch (t.dataset.act) {
      ...
      case 'sick': {
        if (!kl.game.isBuilt('hospital')) { kl.toast?.('先に病院を建ててください'); break; }
        state.sick = MAIN_IDS[Math.floor(Math.random() * MAIN_IDS.length)];
        kl.updateNpcPositions(true); refresh();
        kl.toast?.(`🏥 ${CHARS[state.sick].name}が病気に。病院でお見舞いを`);
        break;
      }
      case 'loverreset': {
        world.lover = null; world.npcCouples = [];
        refresh(); kl.toast?.('💔 恋人・カップルをリセットした');
        break;
      }
    }
  });
