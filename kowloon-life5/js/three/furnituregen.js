...
// プラモデル (ジンパチの趣味): ガラスケース内のロボと戦艦の展示棚
G.jin_plamo = () => {
  const g = grp(
    box(1.4, 1.3, 0.6, 0x3a3a48, { y: 0.65 }),                 // 棚
    box(1.32, 0.04, 0.55, 0x4a4a5a, { y: 0.66 }),
    box(1.36, 1.2, 0.02, 0xbfe2ee, { y: 1.0, z: 0.3, transparent: true, opacity: 0.28, roughness: 0.05 }), // ガラス扉
  );
  // 上段: 組み上がったロボプラモ(赤/青/黄のヒーロー配色)
  const robo = grp(
    box(0.22, 0.3, 0.14, 0xd83a3a, { y: 0.18 }),               // 胴
    box(0.13, 0.13, 0.12, 0xf0e0d0, { y: 0.4 }),               // 頭
    box(0.05, 0.06, 0.02, 0xffe45a, { y: 0.42, z: 0.07 }),     // バイザー
    box(0.07, 0.22, 0.07, 0x3a5ad8, { x: -0.17, y: 0.14 }),    // 腕
    box(0.07, 0.22, 0.07, 0x3a5ad8, { x: 0.17, y: 0.14 }),
    box(0.08, 0.22, 0.08, 0x2a3a8a, { x: -0.07, y: -0.08 }),   // 脚
    box(0.08, 0.22, 0.08, 0x2a3a8a, { x: 0.07, y: -0.08 }),
    box(0.03, 0.28, 0.03, 0xcccccc, { x: 0.26, y: 0.16, rz: 0.3 }), // ビームサーベル
  );
  robo.position.set(-0.35, 1.06, 0);
  g.add(robo);
  // 下段: 戦艦プラモ
  g.add(grp(
    box(0.8, 0.1, 0.2, 0x8a8a92, { y: 0 }),
    box(0.5, 0.08, 0.14, 0x9a9aa2, { y: 0.08 }),
    box(0.06, 0.16, 0.05, 0x6a6a72, { x: -0.05, y: 0.17 }),
    cyl(0.015, 0.015, 0.2, 0x555, { x: 0.2, y: 0.2 }),
  ).translateX(0.3).translateY(0.5));
  // ニッパーとランナー(作りかけ)
  g.add(box(0.3, 0.02, 0.2, 0x6a5a3a, { x: 0.35, y: 0.67, z: 0.18 }));
  return g;
};
